...
(function() {
  ...
  const colors = ['rgba(61,232,196,', 'rgba(27,191,160,', 'rgba(100,200,220,', 'rgba(255,255,255,'];
  ...
  function initParticles() {
    ...
    for (let i = 0; i < 5; i++) {
      jellyfish.push({
        x: rand(50, w - 50), y: rand(h * 0.2, h * 0.8),
        size: rand(15, 35), phase: rand(0, Math.PI * 2),
  ...
  function draw() {
    ...
    for (let j of jellyfish) {
      ...
      j.phase += j.speed;
      j.y += j.vy;
      if (j.y + j.size < 0) { j.y = h + j.size; j.x = rand(50, w - 50); }
    }
...
document.querySelectorAll('.reveal, .impact-item, .timeline-item, .strip-bar-fill').forEach(el => {
  observer.observe(el);
});
...
window.addEventListener('scroll', () => {
  ...
  if (window.scrollY > 60) { nav.style.padding = '0.7rem 4rem'; }
